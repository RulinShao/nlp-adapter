...
class SplitImageNet:
    def __init__(self, path, batch_size, workers, num_tasks):
        ...
        train_datasets, val_datasets = self._construct_dataset_splits(
            train_dataset, val_dataset, num_tasks
        )
        ...
        for td, vd in zip(train_datasets, val_datasets):
            self.train_loaders.append(
                torch.utils.data.DataLoader(
                    td, batch_size=batch_size, shuffle=True, **kwargs
                )
            )

            self.val_loaders.append(
                torch.utils.data.DataLoader(
                    vd, batch_size=batch_size, shuffle=False, **kwargs
                )
            )
    ...
    def _split_dataset(self, dataset, class_split_order, num_tasks):
        task_length = len(class_split_order) // num_tasks
        # assert task_length == self.task_length

        # Used to map from random task_length classes in {0...1000} -> {0,1...task_length}
        tiled_class_map = np.tile(np.arange(task_length), num_tasks)
...
def get_SplitImageNet(path, batch_size, workers, num_tasks):
    dataset = SplitImageNet(path, batch_size, workers, num_tasks)
    return dataset.train_loaders, dataset.val_loaders