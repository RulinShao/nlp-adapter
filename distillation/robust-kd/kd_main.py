...
def main():
    ...
    # Get dataloader.
    data_loader = getattr(datasets, args.set)()
    ...
    for idx in range(args.num_tasks or 0):
        ...
        # Update the datasets loader so that it returns the datasets for the correct task, also done by passing the task index.
        assert hasattr(
            data_loader, "update_task"
        ), "[ERROR] Need to implement update task method for use with multitask experiments"
        ...
        if num_tasks_learned in args.eval_ckpts or num_tasks_learned == args.num_tasks:
            ...
            for i in range(num_tasks_learned):
                ...
                # Update the datasets loader so it is returning datasets for the right task.
...
def get_model(args):
    ...
    if "vit" in args.teacher_model:
        teacher = create_model(args.teacher_model,
                             pretrained=True,
                             num_classes=1000,
                             in_chans=3, )
    else:
        teacher = getattr(torchvision.models, args.teacher_model)(pretrained=True)
    student = getattr(torchvision.models, args.student_model)(pretrained=False)
